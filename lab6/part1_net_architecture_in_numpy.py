...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        def __init__(self, n_in, n_hidden, n_out):
            self.hidden_layer = DenseLayer(n_in=n_in, n_out=n_hidden, f_act=hardlim)
            self.output_layer = DenseLayer(n_in=n_hidden, n_out=n_out, f_act=hardlim)

        def forward(self, x_data):
            output = self.hidden_layer.forward(x_data)
    ...
    model.hidden_layer.W[:, 0] = [-1.5, -4]     # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [1, 2]    # wagi neuronu h2
    model.hidden_layer.b[:] = [4, -0.5]        # biasy neuronów h1 i h2
